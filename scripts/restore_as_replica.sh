...
if [ -x "$PG_BIN/pg_basebackup" ]; then
	su - postgres -c "PGPASSFILE=$PGPASSFILE $PG_BIN/pg_basebackup -h $PRIMARY_HOST -p $PRIMARY_PORT -D $PGDATA -U $REPLICATION_USER -v -P --wal-method=stream" || echo "[restore] Ошибка pg_basebackup (bin)"
else
	su - postgres -c "PGPASSFILE=$PGPASSFILE pg_basebackup -h $PRIMARY_HOST -p $PRIMARY_PORT -D $PGDATA -U $REPLICATION_USER -v -P --wal-method=stream" || echo "[restore] Ошибка pg_basebackup (альтернатива)"
fi
